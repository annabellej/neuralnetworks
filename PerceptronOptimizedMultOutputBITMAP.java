 ...
 *
 * Description: A multilayer Perceptron connectivity model with
...
public class PerceptronOptimizedMultOutputBITMAP
{
   ...
   /*
    * Constructor for a multilayer Perceptron.
    *
    ...
    */
   public PerceptronOptimizedMultOutputBITMAP(int numInputs, int numOutputs, int[] hiddenLayerNodes, double lambda,
   ...
   public void randomizeWeightValues(double low, double high)
   {
      for (int m = 0; m < weights.length; m++)
      {
         for (int source = 0; source < weights[m].length; source++)
         {
            ...
            {
               weights[m][source][dest] = (Math.random()*(high-low))+low;
            }
         }
      }
   }
   ...
   /*
    * The propagation rule calculates the dot product of all the connections
    * into the unit in question.
    *
    ...
    * also saves theta values for training.
    *
    ...
    */
   public void training(double errorThreshold, int iterationThreshold, double changeThreshold, double initError,
                                 double[][] inputVals, double[][] trainingVals, int numTestCases, PrintWriter outputFile)
   {
              ...
              learningFactor!=0.0 && errorChange>changeThreshold))
      {
         for (int t = 0; t < numTestCases; t++) //iterate through each test case
         {
            ...
            //recalc weights: hidden layers (iterate layers backwards, store seniorOmega based on psi of last layer)
            for (int m = numLayers-3; m >= 0; m--)
            {
               for (int k = 0; k < activations[m].length; k++)
               {
                  for (int j = 0; j < activations[m+1].length; j++)
                  {
                     double psiJ = seniorOmegaVals[m+1][j] * derivActivationFunction(thetaVals[m+1][j]);

                     weights[m][k][j] += learningFactor * activations[m][k] * psiJ;

                     seniorOmegaVals[m][k] += psiJ * weights[m][k][j];
                  }
               }
            }
   ...
   public static void main(String[] args) throws IOException
   {
      ...
      PerceptronOptimizedMultOutputBITMAP testNet = new PerceptronOptimizedMultOutputBITMAP(numInputs, numOutputs, hiddenLayerNodes,
      ...
      testNet.training(errorThreshold, iterationThreshold, changeThreshold, initError, inputVals,
              trainingVals, numTestCases, out2);

      //print outputs
      int count = 0;
      for (int i = 0; i < 50; i++) //TAKE OUT THESE HARD CODED ROW/COL NUMBERS
      {
         for (int j = 0; j < 50; j++)
         {
            out2.print(testNet.activations[testNet.numLayers-1][count]+" ");
            count++;
         }
         out2.print("\n");
      }
