...
// inverted format with a positive bmpInfoHeader_biHeight value.
   ...
   /*
    * The rgbqToPel method takes red, green and blue color values plus an additional byte and
    * returns a single 32-bit integer color.
    *
    ...
    * using just one of may possible formulas
    */
...
// The color table
      ...
      BitmapRead dibdumper = new BitmapRead(); // needed to get to the byte swapping methods

      PrintWriter pelOut = new PrintWriter(new BufferedWriter(new FileWriter("pelOut.out")));
      ...
      for (int testCases = 0; testCases < 5; testCases++)
      {
         ...
         if (args.length > 0)
            ...
            inFileName = "hand0."+(testCases+1)+".bmp";

         if (args.length > 1)
            outFileName = args[1];
         else
            outFileName = "outhand"+(testCases+1)+".bmp";
...
// System.out.printf("DEBUG: Color Table = %d, %06X\n", i, colorPallet[i]);
                  ...
                  for (int row = 0; row < bmpInfoHeader_biHeight; ++row) // read over the rows
                  {
                     ...
                     for (j = 0; j < iBytesPerRow; ++j) {
                        ...
                        for (k = 0; k < 8; ++k)     // Get 8 pels from the one byte
                        {
                           ...
                           imageArray[i][iColumn] = ~dibdumper.colorToGrayscale(pel);
                        }
                     }
               ...
               case 8: // 1 byte, 1 pel, Works
                  ...
                  for (int row = 0; row < bmpInfoHeader_biHeight; ++row) // read over the rows
                  {
                     ...
                     for (j = 0; j < iPelsPerRow; ++j)         // j is now just the column counter
                     {
                        ...
                        imageArray[i][j] = ~dibdumper.colorToGrayscale(pel);
                     }
         ...
         //make new image array with scaled 0 to 1 pel values
         double[][] imageArrayScaled = new double[30][30];
         for (int r = 0; r < imageArray.length; r++) {
            for (int c = 0; c < imageArray[r].length; c++) {
               imageArrayScaled[r][c] = ((imageArray[r][c] / 1.7e7) + 1);
            }
         }

         //write scaled pel values out to a file to be used as input activations
         for (int r = 0; r < imageArrayScaled.length; r++) {
            for (int c = 0; c < imageArrayScaled[r].length; c++) {
               pelOut.print(imageArrayScaled[r][c] + " ");
            }
         }
         pelOut.print("\n");
         pelOut.close();
...
// there is no color table for this true color image, so write out the pels
      ...
      //run network on pels as inputs
      PerceptronOptimizedMultOutputHANDS.main(args);
